...
One of the key variable in this dataset is the "Average Actual speed". This has NAs for the countries Hungary, Poland, Slovak and Turkey. This is interesting because these countries do have values in the Average Download speed, upload speed and other variables.
Also to note are the Max value of this variable, 15239Kbps for Korea and the min value 948Kbps for Mexico. 

While briefly looking at the other key variables we noticed that in the measurement of the standard deviation of latency for Iceland, Iceland has an unusually high value of 1,199 (where all the other standard deviations are around 300). The standard deviation for upload speed is 12098 for Sweden. Again unsually high while the others have SD in the range of 4000. 

We also notice that the max values for downloads are for Korea and max values for uploads are for Japan.

Let's look at the distribution of the key variable using an histogram:

```{r}
hist(df_speed$`Average Actual`, breaks=20, col="red")
```

####Let's look at our final dataset
```{r}
colnames(df_penetration)[3:12] <- c("Penetration 2008", "Penetration 2007", "Household Penetration" ,"2G and 3G Penetration", "Penetration GC", "3G Penetration", "Growth in 3G" , "WiFi hotspots" , "WiFi per 100000", "Percent Urban" )
...
```{r}
boxplot(df_penetration[3:8], las=2, names=c("2008", "2007", "Household" , "2G & 3G", "GC", "3G"))
```
The above boxplot shows that the data is pretty solid and can be used as is for further analysis. Let's look at the boxplot of the remaining variables.
```{r}
boxplot(df_penetration[9])
```
```{r}
boxplot(df_penetration[10])
```
```{r}
boxplot(df_penetration[11])
```
```{r}
boxplot(df_penetration[12])
```
