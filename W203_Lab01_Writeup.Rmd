...
```
The other Key variable in this dataset is "Average Download speed"
```{r}
hist(df_speed$`Average Actual`, breaks=20, col="royalblue1", main = 'Histogram of Average Download Speed, Akamai', xlab = 'Average Download Speed, Akamai (kbps)')
```
While realizing that the Actual speed has some missing values and looking at the above histogram and it's similarity to the histogram of the "Average Actual speed", we have decided to use "Average Download speed" as the proxy speed variable for further analysis.
...
```
The above boxplot shows that the penetration data for 2008, 2007 and GC are highly correlated and and can be used as is for further analysis. We chose the 2008 OECD penetration data as the proxy for the penetration in the Key relationship analysis. 
...
In order to perform the Key relationship analysis, we chose the "price for med speeds" from the Price dataset, "2008 Penetration OECD" from the Penetration dataset and "Average Download Speed" from the speed dataset.

```{r}
scatterplotMatrix(~df_full$Price.for.med.speeds..combined + df_full$Penetration.per.100.OECD..2008 + df_full$Average.download.speedtest.net..kbps., data=df_full)
```
First Observation: When penetration increases the price comes down

```{r}
pairs(~df_full$Price.for.med.speeds..combined+df_full$Average.download.speedtest.net..kbps.+df_full$Penetration.per.100.OECD..2008)
```
...
Secondary variables
Culture: South Korea - high speed, gaming / Internet is big
Wealth of country - can afford more infrastructure
Result: need more data, take with a grain of salt.
